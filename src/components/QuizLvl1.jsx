...
import { questionsLvl1 } from "../assets/data";
import { Link } from "react-router-dom";

function QuizLvl1() {
  const [index, setIndex] = useState(0);
  const [question, setQuestion] = useState(questionsLvl1[index]);
  ...
  const goNextQuestion = () => {
    if (index == questionsLvl1.length - 1) {
      setIsEnded(true);
    }
    if (lock) {
      setIndex((prevIndex) => prevIndex + 1);
      setQuestion(questionsLvl1[index + 1]);
      setLock(false);
      answers.map((a) => {
        ...
        <>
          ...
          <div className="flex items-center justify-between mt-10 font-serif px-3">
            ...
            <p className="font-thin text-xl font-serif">
              Question {index + 1}/{questionsLvl1.length}
            </p>
